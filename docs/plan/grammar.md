...
```
Canonical: `title:` is the third line (after pragma and family). `direction` is folded
into the family line. Directives elsewhere in the file are hoisted, warning W104 if a
directive repeats (first wins).
...
Blocks are frames in the scene with `metadata.dsl.block = loop|alt|…`.
...
```
`[*]` is a token, never a name. Canonical: §6.4 sections (pseudo-state declarations,
composites as blocks, then transitions).
...
```
Row = `name [type] [pk|fk|unique|nullable]*`, canonical spacing single. Cardinality words
(`1:N`, `one or many`) accepted, canonical is glyphs. Entity node kind `er_entity`.
...
emitted only when a class has both attributes and methods.
Canonical: classes in line order as blocks, relations after.
...
- Retention
```
Indentation (2 or 4 spaces, tabs = 4) is structure; `-` or `*` bullet optional on input,
canonical is `- ` with 2-space indent. Max depth 6, deeper collapses (W140). Colour on a
branch cascades. No edges; `->` lines are W111.
...
```
Imperative; canonical = line order. `main` exists implicitly. Unknown branch → W150.

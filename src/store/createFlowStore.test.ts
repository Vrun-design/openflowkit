...
describe('createFlowStore', () => {
  ...
  it('deletes multiple documents atomically and selects one surviving workspace', () => {
    const store = createFlowStore();
    const firstId = store.getState().createDocument();
    const secondId = store.getState().createDocument();
    const thirdId = store.getState().createDocument();

    store.getState().deleteDocumentRecords([firstId, thirdId, 'missing']);

    const state = store.getState();
    expect(state.documents.map((document) => document.id)).toEqual([secondId]);
    expect(state.activeDocumentId).toBe(secondId);
    expect(state.tabs).toHaveLength(1);
    expect(state.activeTabId).toBe(state.documents[0]?.activePageId);
  });

  it('clears canvas and selection when bulk deletion removes every document', () => {
    const store = createFlowStore();
    const firstId = store.getState().createDocument();
    const secondId = store.getState().createDocument();
    store.setState({ selectedNodeId: 'node', selectedEdgeId: 'edge' });

    store.getState().deleteDocumentRecords([firstId, secondId]);

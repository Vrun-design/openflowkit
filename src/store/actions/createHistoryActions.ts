...
export function buildHistorySnapshot(state: FlowState): FlowHistoryState {
...
export function findActiveTabIndex(state: FlowState): number {
...
function trimSnapshots(
  ...
  const trimmed =
    direction === 'past' ? snapshots.slice(-MAX_HISTORY) : snapshots.slice(0, MAX_HISTORY);
...
export function trimPastSnapshots(snapshots: FlowHistoryState[]): FlowHistoryState[] {
...
> {
  return {
    ...
    scrubHistoryV2: (targetIndex) => {
      set((state) => {
        const activeTabIndex = findActiveTabIndex(state);
        if (activeTabIndex < 0 || !Number.isFinite(targetIndex)) return {};

        const activeTab = state.tabs[activeTabIndex];
        const current = buildHistorySnapshot(state);
        const timeline = [...activeTab.history.past, current, ...activeTab.history.future];
        const currentIndex = activeTab.history.past.length;
        const boundedTargetIndex = Math.max(
          0,
          Math.min(Math.trunc(targetIndex), timeline.length - 1)
        );
        if (boundedTargetIndex === currentIndex) return {};

        const target = timeline[boundedTargetIndex];
        const updatedTab = {
          ...activeTab,
          history: {
            past: timeline.slice(0, boundedTargetIndex),
            future: timeline.slice(boundedTargetIndex + 1),
          },
          nodes: target.nodes,
          edges: target.edges,
          layers: target.layers ?? state.layers,
          canvasExtensions: target.canvasExtensions,
        };
        const tabs = [...state.tabs];
        tabs[activeTabIndex] = updatedTab;

...
function matchesLoadedPages(
  currentPages: FlowTab[],
  documentPages: FlowDocument['pages']
): boolean {
  ...
  return {
    ...
    renameDocument: (id, nextName) => {
      ...
      set((state) => {
        ...
        const shouldMirrorToSinglePage =
          target.pages.length === 1 && target.pages[0]?.name === target.name;
    ...
    deleteDocumentRecord: (id) => get().deleteDocumentRecords([id]),
    deleteDocumentRecords: (ids) => {
      ...
      set((state) => {
        ...
        if (remainingDocuments.length === state.documents.length) return {};

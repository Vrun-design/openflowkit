...
import { useActiveCanvas, useActiveCanvasViewport } from '@/canvas/activeCanvas';
...
export function NavigationControls(): React.ReactElement {
  ...
  const { zoomIn, zoomOut, fitView } = useActiveCanvas();
  const { zoom } = useActiveCanvasViewport();
    ...
    <div className="absolute bottom-8 left-4 z-50">
      <div className="flex flex-col rounded-[var(--radius-lg)] border border-[var(--color-brand-border)] bg-[var(--brand-surface)] p-1 shadow-[var(--shadow-md)] ring-1 ring-black/5">
        ...
        <div
          ...
          className="select-none py-1 text-center text-[10px] font-medium tabular-nums text-[var(--brand-secondary)]"
        >
          {Math.round(zoom * 100)}%
        </div>

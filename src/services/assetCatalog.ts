...
const PROVIDER_BACKED_ASSET_CATEGORIES: DomainLibraryCategory[] = [
  'aws',
  'azure',
  'gcp',
  'cncf',
...
function getEmbeddedIconAssetItems(): DomainLibraryItem[] {
  return DOMAIN_LIBRARY_ITEMS.filter(
    (item) => item.category === 'network' || item.category === 'c4' || item.category === 'security'
  ).map((item) => ({
    ...item,
    category: 'icons',
    assetPresentation: 'icon',
    providerShapeCategory:
      item.category === 'network' ? 'Network' : item.category === 'c4' ? 'C4' : 'Security',
  }));
}

export function loadDomainAssetCatalog(
  category: DomainLibraryCategory
): Promise<DomainLibraryItem[]> {
  if (category === 'icons') {
    return Promise.resolve([...loadIconAssetCatalog(), ...getEmbeddedIconAssetItems()]);
  }
...
export function loadDomainAssetSuggestions(
  ...
  if (category === 'icons') {
    ...
    const items = [...loadIconAssetCatalog(), ...getEmbeddedIconAssetItems()];
    ...
    const filteredItems = items.filter((item) => {
      ...
      if (
        ...
        item.providerShapeCategory !== options.category
      ) {
        return false;
      }
      if (!normalizedQuery) {
...
export async function resolveDomainLibraryItemForInsertion(
  ...
  return preview ? { ...item, previewUrl: preview.previewUrl } : item;
}
